...
class PersonaRemoteRepository @Inject constructor(
    ...
    suspend fun obtenerPersonaPorId(id: Int): Result<PersonaDTO?> {
        return try {
    ...
    // Actualizar persona
    suspend fun actualizarPersona(id: Int, personaDTO: PersonaDTO): Result<PersonaDTO?> {
        return try {
    ...
    // Desactivar persona
    suspend fun eliminarPersona(id: Int): Result<Boolean> {
        return try {
