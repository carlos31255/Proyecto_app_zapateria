...
class RolRemoteRepository @Inject constructor(
    ...
    // Obtener rol por ID
    suspend fun obtenerRolPorId(id: Int): Result<RolDTO?> {
        return try {
    ...
    // Actualizar rol
    suspend fun actualizarRol(id: Int, rolDTO: RolDTO): Result<RolDTO?> {
        return try {
    ...
    // Eliminar rol
    suspend fun eliminarRol(id: Int): Result<Boolean> {
        return try {
